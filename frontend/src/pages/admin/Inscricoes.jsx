// pages/admin/Inscricoes.jsx
// pages/coordenador/Inscricoes.jsx
...
function ModalInscreverParticipante({ formacaoId, formacaoTitulo, onClose, onSuccess }) {
  ...
  useEffect(() => {
    api.get('/usuarios?tipo=participante&p=1')
      ...
      .catch(() => toast('Erro ao carregar participantes.', 'erro'))
      .finally(() => setLoadingPart(false));
  }, [toast]);

  const filtrados = participantes.filter(p => {
    const termo = buscaPart.toLowerCase();

    return (
      !buscaPart ||
      String(p.nome_completo || '').toLowerCase().includes(termo) ||
  ...
  async function inscrever() {
    ...
    try {
      ...
      const nome =
        participantes.find(p => p.id === parseInt(usuarioId))?.nome_completo || '';
...
export default function AdminInscricoes() {
  ...
  async function carregar(id) {
    ...
    try {
      ...
      toast('Erro ao carregar inscrições.', 'erro');
    } finally {
      setLoading(false);
    }
  }
...
>
  ➕ Inscrever Participante
      ...
      {loading ? (
        ...
        <div className="vazio">
          <div className="vazio-icone">📋</div>
          <p>Selecione uma formação para ver as inscrições.</p>
        </div>
      ) : (
