...
export default function PropostaFormacaoDetalhe() {
  ...
  const rotaListaPropostas = '/admin/propostas-formacao';
  ...
  function mostrarTurnos(turnos) {
    ...
    return turnos.map(turno => labels[turno] || turno).join(', ');
  }

  function mostrarHorarioTurno(turno) {
    const inicio = proposta[`hora_inicio_${turno}`];
    const fim = proposta[`hora_fim_${turno}`];
  ...
  return (
    <PainelLayout titulo="Proposta de Formação">
      <div className="d-flex justify-between mb-24">
        ...
        <Link className="btn btn-outline" to={rotaListaPropostas}>
          Voltar
        </Link>
      </div>
      ...
      <div className="card mb-24">
        ...
        <p><strong>Setor demandante:</strong> {proposta.setor || '—'}</p>
      ...
      <div className="card mb-24">
        ...
        <p><strong>Data inicial:</strong> {formatarData(proposta.data_encontro)}</p>
        <p><strong>Data final:</strong> {formatarData(proposta.data_fim || proposta.data_encontro)}</p>
        ...
        {proposta.turnos?.includes('manha') && (
          <p><strong>Horário da manhã:</strong> {mostrarHorarioTurno('manha')}</p>
        )}
        {proposta.turnos?.includes('tarde') && (
          <p><strong>Horário da tarde:</strong> {mostrarHorarioTurno('tarde')}</p>
        )}
        {proposta.turnos?.includes('noite') && (
          <p><strong>Horário da noite:</strong> {mostrarHorarioTurno('noite')}</p>
        )}
        <p><strong>Chegada dos organizadores:</strong> {proposta.hora_chegada ? String(proposta.hora_chegada).slice(0, 5) : '—'}</p>
      </div>
      ...
      {proposta.status === 'pendente' && (
        <div className="card">
          <h3 className="card-titulo">Análise administrativa</h3>
