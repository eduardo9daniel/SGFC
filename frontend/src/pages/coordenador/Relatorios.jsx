...
// ─── Formata timestamp para nome de arquivo: YYYY-MM-DD_HHmmss ─────────
...
// ─── Estilo temporário aplicado somente durante a geração do PDF ─────────
...
export default function CoordRelatorios() {
  ...
  async function gerar(e) {
    ...
    try {
      const params = new URLSearchParams({
        ...
        data_fim: dataFim,
        ...(formacaoSel && { formacao_id: formacaoSel })
      });

      const { data } = await api.get('/relatorios?' + params.toString());
      setDados(data.data);
      ...
      try {
        ...
        dadosDashboard.acessos_por_regiao || []
);
      } catch (errDash) {
        console.error('Erro ao carregar pessoas por região:', errDash);
        setDashboardStats({});
        setPessoasRegiaoDados([]);
      }
    } catch (err) {
  ...
  async function exportarPDF() {
    ...
    try {
      ...
      pdf.save(`relatorio_coord_${timestampArquivo()}.pdf`);
    } catch (err) {
  ...
  const totalStatus = dados?.distStatus?.reduce(
  ...
  const lista = dados?.topFormacoes || [];
        ...
        <div ref={relatorioRef}>
          <div className="stats-grid mb-24">
            {[
              {
                icon: '📋',
                valor: dados.estatsPeriodo?.inscricoes || 0,
                label: 'Inscrições no Período',
                cor: 'laranja'
              },
              {
                icon: '👥',
                valor: dados.estatsPeriodo?.participantes || 0,
                label: 'Participantes Únicos',
                cor: 'verde'
              },
              {
              icon: '📚',
              valor: dados.estatsPeriodo?.formacoes || 0,
          ...
          <div className="grid-pdf-duas-colunas">
            <div className="card">
              ...
              {dados.topFormacoes?.length === 0 ? (
                <div className="vazio">
                  <p>Nenhum dado para o período.</p>
                </div>
              ) : (
                dados.topFormacoes?.map(tf => {
            ...
            <div className="card">
              ...
              {dados.distStatus?.map(ds => {
                const pct = totalStatus > 0
                  ? Math.round((Number(ds.total) / totalStatus) * 100)
                  : 0;

                return (
                  <div key={ds.status} className="linha-status" style={{ marginBottom: 12 }}>
                    <div
                      style={{
                        display: 'flex',
                        justifyContent: 'space-between',
          ...
          {dados.relInscricoes?.length > 0 && (
            <div className="card p-0">
              ...
              >
                ...
                >
                  {dados.relInscricoes.length} participante(s)
                </span>
              </div>
